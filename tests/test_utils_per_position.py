...
def test_single_values_broadcast(value):
    ...
    assert per_position(value, 4) == (value,) * 4
...
def test_full_length_sequences_pass_through(seq):
    ...
    assert per_position(seq, 3) == (0.1, 0.2, 0.3)
...
def test_masked_sequence_scatters_onto_marked_positions():
    ...
    assert per_position((10, 20, 30), 7, mask=mask) == (10, 10, 10, 20, 30, 10, 10)
...
def test_wrong_length_names_both_accepted_counts():
    ...
    with pytest.raises(ValueError) as excinfo:
        per_position((1, 2, 3), 5, "attn_n_heads", mask, "[d0, d1, mid, u0, u1]")
...
def test_args_expand_into_one_dict_per_position():
    ...
    expanded = per_position_args({"res_dropout": (0.0, 0.5), "res_groups": 8}, 2)
    assert expanded == [
...
def test_expanded_dicts_are_independent():
    ...
    expanded = per_position_args({"res_groups": 8}, 3)
