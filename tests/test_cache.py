...
def test_estimate_dict_size():
    """Test the estimate_dict_size function."""
    test_dict = {
        "numbers": [1, 2, 3, 4, 5],
        "index": 1,
        "bool": False,
    ...
    dct_size = estimate_dict_size(test_dict)
    print(dct_size)
...
def test_SharedDict_init(
    ...
    cache_dict = SharedDict(descr, size=cache_size, verbose=True)
    print(cache_dict)
...
def test_SharedDictList_init(
    ...
    cache_dict = SharedDictList(descr, size=cache_size, verbose=True)
    print(cache_dict)
